...
When you write a formula with
[`openxlsx2::wb_add_formula()`](https://janmarvin.github.io/openxlsx2/reference/wb_add_formula.html),
the string goes directly into the XML. Excel 365 functions like
`LAMBDA`, `XLOOKUP`, `FILTER`, or `SEQUENCE` need namespace prefixes in
storage that you never type in the formula bar:
...
arrays. Write them in plain formula-bar syntax;
[`to_xml()`](https://janmarvin.github.io/spaghetti/reference/to_xml.md)
adds the required prefixes before handing off to openxlsx2. Works with
both single formulas and vectors.
...
wb <- wb |>
  ...
  wb_add_formula(
    dims = "F2",
    x = to_xml("=MAP(E2#, LAMBDA(p, SUM((B2:B7=p)*C2:C7)))"),
    cm = TRUE
  )
...
format with all prefixes intact.
[`from_xml()`](https://janmarvin.github.io/spaghetti/reference/from_xml.md)
strips them back to readable form — useful for auditing, diffing, or
displaying formulas to users.
...
| [`supported_locales()`](https://janmarvin.github.io/spaghetti/reference/supported_locales.md) | — | List available locale codes |
...
`SUMIF`), pass the `locale` argument to
[`to_xml()`](https://janmarvin.github.io/spaghetti/reference/to_xml.md)
to translate first.

**Setup required**: run
[`setup_terminology()`](https://janmarvin.github.io/spaghetti/reference/setup_terminology.md)
once per machine to download the function-name translation data. Without
that, locale calls will error.
...
validates function names and suggests corrections for typos:
